...
AS
BEGIN
	UPDATE tbl_Employee SET ActualRankID = dbo.fn_GetActualRankID(i.EmployeeID), ActualPostID = dbo.fn_GetActualPostID(i.EmployeeID)
    FROM (SELECT DISTINCT EmployeeID FROM INSERTED ) i;
  
	UPDATE tbl_Employee SET ActualRankID = dbo.fn_GetActualRankID(d.EmployeeID), ActualPostID = dbo.fn_GetActualPostID(d.EmployeeID)
    FROM (SELECT DISTINCT EmployeeID FROM DELETED ) d;
